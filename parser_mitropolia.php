<?php
/**
 * Ïàðñåð mitropolia-simbirsk.ru -> host1409556_barysh.news_mitropolia
 *
 * Îïòèìèçèðîâàííàÿ âåðñèÿ:
 *  - ìèíèìàëüíàÿ íàãðóçêà íà CPU/ñåòü
 *  - íå ä¸ðãàåò ëèøíèé ðàç óäàë¸ííûé ñàéò
 *  - íå ñêà÷èâàåò ïîâòîðíî èçîáðàæåíèÿ, åñëè çàïèñü óæå åñòü
 *  - îãðàíè÷åíèå îáùåãî âðåìåíè âûïîëíåíèÿ ñêðèïòà
 */

error_reporting(E_ALL ^ E_NOTICE);
ini_set('display_errors', 0); // â êðîíå íàì íå íóæåí âûâîä âîðíèíãîâ
header('Content-Type: text/plain; charset=utf-8');

/* ==== Îãðàíè÷åíèå îáùåãî âðåìåíè ðàáîòû ñêðèïòà ==== */
$script_start = microtime(true);
$max_runtime  = 20; // ìàêñèìóì 20 ñåêóíä íà âåñü ïàðñèíã
...
/**
 * Áûñòðûé HTTP GET ñ íåáîëüøèìè òàéìàóòàìè,
 * ÷òîáû íå çàâèñàòü è íå ãðóçèòü ñèñòåìó.
 */
function http_get($url){
    if(function_exists('curl_init')){
        ...
        curl_setopt_array($ch, array(
            ...
            CURLOPT_CONNECTTIMEOUT => 3,  // ðàíüøå áûëî 15
            CURLOPT_TIMEOUT        => 5,  // ðàíüøå áûëî 30
        ...
        $data = curl_exec($ch);
        if($data === false){
            // ìîæíî çàëîãèðîâàòü curl_error($ch) ïðè íåîáõîäèìîñòè
            curl_close($ch);
            return '';
        }
        curl_close($ch);
...
/**
 * Ïîëó÷èòü og:image / twitter:image / ïåðâóþ êàðòèíêó.
 * Èñïîëüçóåò http_get ñ êîðîòêèìè òàéì-àóòàìè.
 */
...
/**
 * Ñêà÷àòü è ñîõðàíèòü êàðòèíêó ëîêàëüíî.
 * Íå âûçûâàåòñÿ, åñëè â ÁÄ óæå åñòü îáëîæêà.
 */
function save_image_local($img_url, $upload_dir, $prefix='img'){
    ...
    if(!$bin || strlen($bin) < 512) return ''; // ìåíüøå 512 áàéò — ìóñîð
...
$cache_ttl = 600; // áûëî 300, òåïåðü 10 ìèíóò — ìåíüøå çàïðîñîâ ê RSS
...
foreach($feeds as $section=>$rss_url){
    ...
    // äëÿ "slovo" ïðèíóäèòåëüíî ñáðàñûâàåì êýø (êàê è áûëî)
    if($section === 'slovo'){
    ...
    foreach($items as $item){
        ...
        if(check_runtime_limit($script_start, $max_runtime)){
            break 2; // âûõîäèì è èç öèêëà ïî ôèäàì òîæå
        }
        ...
        if($ts < $cutoff_ts){
            // RSS îáû÷íî îòñîðòèðîâàí ïî äàòå, äàëüøå òîëüêî ñòàðü¸
            break;
        }

        $date = date("Y-m-d H:i:s",$ts);

        // ÑÐÀÇÓ ïðîâåðÿåì, åñòü ëè òàêàÿ çàïèñü â ÁÄ
        ...
        if(!$img){
            ...
            // 5) ïåðâàÿ êàðòèíêà èç ïîëíîãî HTML
            if(!$img && $link){
        ...
        // Ñêà÷èâàåì êàðòèíêó ÒÎËÜÊÎ åñëè:
        //  - åñòü URL êàðòèíêè
        //  - è ëèáî çàïèñè íåò, ëèáî ó íå¸ ïóñòàÿ îáëîæêà
